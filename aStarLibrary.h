...
#define unwalkable 1
	...
	node walkability [mapWidth][mapHeight];
...
// 		whether a cell is on the open list or on the closed list.
    ...
    int pathBank[numberPeople+1][100];
...
{
    int onClosedList = 10;// walkability array constants
    int x, y;
...
//	If target square is unwalkable, return that it's a nonexistent path.
	if (walkability[targetX][targetY].walkability == unwalkable){
...
//	If not a wall/obstacle square.
	if (walkability [a][b].walkability != unwalkable) { 
...
//	Don't cut across corners
	...
	if (a == parentXval-1) 
	{
		if (b == parentYval-1)
		{
			if (walkability[parentXval-1][parentYval].walkability == unwalkable
				|| walkability[parentXval][parentYval-1].walkability  == unwalkable) \
				corner = unwalkable;
		}
		else if (b == parentYval+1)
		{
			if (walkability[parentXval][parentYval+1].walkability  == unwalkable
				|| walkability[parentXval-1][parentYval].walkability  == unwalkable) 
				corner = unwalkable; 
		}
	}
