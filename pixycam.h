...
extern int sendByte(uint8_t byte) {
    if (!(DRV_USART_TRANSFER_STATUS_TRANSMIT_FULL & DRV_USART_TransferStatus(pixyHandle)) )
    {
        DRV_USART_WriteByte(pixyHandle, byte);
    }
}
...
extern uint8_t getByte(void) {
    ...
    while(PLIB_USART_ReceiverDataIsAvailable(USART_ID_2)) {
        u = PLIB_USART_ReceiverByteReceive(USART_ID_2);
        if(u >= 0)
            break;
    }
    
    /*
	while(!DRV_USART_ReceiverBufferIsEmpty(pixyHandle))
    {
      u = DRV_USART_ReadByte(pixyHandle);
      if (u>=0)
        break;
    }
    */
